<template>
	<view class="content">
		<view class="content-slideshow">
			<uni-swiper-dot :info="[0,1]" :current="current" field="content" mode="round" :dots-styles="dotsStyles">
				<swiper @change="change" class="remake-swiper-item">
					<swiper-item>
						<image class="swiper-item" src="../../static/index/slideshow-1.jpeg" mode="widthFix" />
					</swiper-item>
					<swiper-item>
						<image class="swiper-item" src="../../static/index/slideshow-1.jpeg" mode="widthFix" />
					</swiper-item>
				</swiper>
			</uni-swiper-dot>
		</view>
		<view class="content-operation">
			<view class="content-operation2">
				<view class="content-operation-item"
					style="background: url(https://dgbroker.in/app/img/deposit_bg@2x.24ebf008.png) no-repeat center center;background-size: contain;">
					<image class="operation-item-icon" src="https://dgbroker.in/app/img/deposit@2x.708eddb6.png"
						mode="aspectFit" />
					<view class="operation-item-name">
						Recharge
					</view>
				</view>
				<view class="content-operation-item"
					style="background: url(https://dgbroker.in/app/img/withdraw_bg@2x.af3b5f8f.png) no-repeat center center;background-size: contain;">
					<image class="operation-item-icon"
						src="data:image/png;base64,"
						mode="aspectFit" />
					<view class="operation-item-name">
						Withdraw
					</view>
				</view>
				<view class="content-operation-item"
					style="background: url(https://dgbroker.in/app/img/invite_bg@2x.30c9b362.png) no-repeat center center;background-size: contain;">
					<image class="operation-item-icon"
						src="data:image/png;base64,"
						mode="aspectFit" />
					<view class="operation-item-name">
						Direct
					</view>
				</view>
				<view class="content-operation-item"
					style="background: url(https://dgbroker.in/app/img/service_bg@2x.7973f282.png) no-repeat center center;background-size: contain;">
					<image class="operation-item-icon" src="https://dgbroker.in/app/img/service@2x.41b858a5.png"
						mode="aspectFit" />
					<view class="operation-item-name">
						Service
					</view>
				</view>
			</view>
		</view>
		<view class="content-widhraw">
			<view class="content-widhraw-title">
				Users Withdraw
			</view>
			<view class="content-widhraw-log">
				<marquee behavior="alternate" scrolldelay="200" direction="up" align="absmiddle" class="mq">
					<view class="item">
						<view class="item-li">MS Yashaswini </view>
						<view class="item-li">600</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Diwakar </view>
						<view class="item-li">3800</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Madhankumar</view>
						<view class="item-li">1000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">K.NAGENDRAN</view>
						<view class="item-li">2000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Krushna </view>
						<view class="item-li">6000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Charan n</view>
						<view class="item-li">1100</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Jaanapaati Prabhakar</view>
						<view class="item-li">600</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Selvakumar.M</view>
						<view class="item-li">1800</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Kalai</view>
						<view class="item-li">2500</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Sivasethu S</view>
						<view class="item-li">300</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Dinesh</view>
						<view class="item-li">500</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Mohan Gaddam</view>
						<view class="item-li">1200</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Radha Krishna</view>
						<view class="item-li">2000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Bhuvaneshwari. P</view>
						<view class="item-li">1200</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">chand basha</view>
						<view class="item-li">500</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Devaki.p</view>
						<view class="item-li">1000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Mahalakshmi m </view>
						<view class="item-li">2500</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Sreeja Sree</view>
						<view class="item-li">5200</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">R sundresh</view>
						<view class="item-li">6800</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Hemanth Kumar</view>
						<view class="item-li">900</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Tejasri K</view>
						<view class="item-li">4700</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Chandu M</view>
						<view class="item-li">2000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Sakthi Kumar</view>
						<view class="item-li">1100</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Murali</view>
						<view class="item-li">2000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Subasri</view>
						<view class="item-li">6500</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Chandra</view>
						<view class="item-li">1650</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Suman</view>
						<view class="item-li">1800</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">shymal a</view>
						<view class="item-li">1500</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Sreenivasan</view>
						<view class="item-li">3500</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Vishwa</view>
						<view class="item-li">2050</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">M SIVAKUMAR</view>
						<view class="item-li">2000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Sharan</view>
						<view class="item-li">3000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Devaki.p</view>
						<view class="item-li">500</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Kanaga Thennarasu </view>
						<view class="item-li">300</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">C Gracy</view>
						<view class="item-li">3000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Janakiraman</view>
						<view class="item-li">5000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Bharathiraja T</view>
						<view class="item-li">300</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">ASHOKKUMAR </view>
						<view class="item-li">300</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Moovarasan Ganapathi</view>
						<view class="item-li">2000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Thennarasi Madhuraj</view>
						<view class="item-li">2000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Harish</view>
						<view class="item-li">1000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Nandhini</view>
						<view class="item-li">300</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Jaanapaati Prabhakar</view>
						<view class="item-li">1200</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">LOGESH KUMAR</view>
						<view class="item-li">1000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Yashwanth</view>
						<view class="item-li">1000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Lavanya </view>
						<view class="item-li">330</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Nanda</view>
						<view class="item-li">2000</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Somiya vijay</view>
						<view class="item-li">1500</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">Rajeswari </view>
						<view class="item-li">2500</view>
						<view class="item-li">Succeed</view>
					</view>
					<view class="item">
						<view class="item-li">James Samuel B </view>
						<view class="item-li">738</view>
						<view class="item-li">Succeed</view>
					</view>
				</marquee>
			</view>
		</view>
	</view>
</template>

<script>
	export default {
		data() {
			return {
				current: 0,
				dotsStyles: {
					backgroundColor: 'rgba(83, 200, 249,0.3)',
					border: '1px rgba(83, 200, 249,0.3) solid',
					color: '#fff',
					bottom: "60",
					selectedBackgroundColor: 'rgba(83, 200, 249,0.9)',
					selectedBorder: '1px rgba(83, 200, 249,0.9) solid'
				}
			}
		},
		onLoad() {

		},
		methods: {
			change(e) {
				this.current = e.detail.current;
			}
		}
	}
</script>

<style lang="scss">
	.content {
		background: #f6f7f9;
		height: 100%;
		padding-top: 20rpx;

		.content-slideshow {
			padding: 0 20rpx;

			.remake-swiper-item {
				height: 440rpx;
			}

			.swiper-item {
				width: 100%;
				height: 100%;
			}
		}

		.content-operation {
			margin: 0 20rpx;
			height: 260rpx;
			position: relative;

			.content-operation2 {
				position: absolute;
				top: -90rpx;
				display: flex;
				justify-content: space-between;
				flex-wrap: wrap;

				.content-operation-item {
					width: 190px;
					height: 80px;
					margin: 10rpx 0;
					display: flex;
					justify-content: center;
					align-items: center;
					border-radius: 20rpx;

					.operation-item-icon {
						width: 80rpx;
						height: 80rpx;
						float: left;
					}

					.operation-item-name {
						width: 180rpx;
						color: #fff;
						font-size: 36rpx;
						padding-left: 36rpx;
						float: left;
					}
				}

			}
		}

		.content-widhraw {
			background: #fff;

			.content-widhraw-title {
				padding: 36rpx 18rpx;
				color: #5ad2fe;
				border-bottom: 1px solid #f2f2f2;
			}

			.content-widhraw-title::before {
				content: " ";
				width: 20rpx;
				background: #5ad2fe;
				float: left;
				height: 36rpx;
				margin-right: 20rpx;
			}

			.content-widhraw-log {
				.mq {
					height: 340rpx;
					background: #fff;
					overflow: hidden;

					.item {
						padding: 20rpx 0;
						display: flex;
						justify-content: center;
						align-items: center;

						.item-li {
							width: 33%;
							text-align: center;
						}
					}
				}

			}
		}
	}
</style>
